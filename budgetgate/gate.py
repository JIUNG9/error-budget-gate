...
def evaluate(action: Action, service: Service, budget_remaining: float,
             env: str = "dev") -> GateDecision:
    ...
    if not reversible:
        return GateDecision(False, False,
                            f"BLOCK: '{action.name}' is not a reversible action")
    if budget_remaining <= 0:
        return GateDecision(False, True,
                            "BLOCK: error budget exhausted — page a human, no automated action")
    if service.tier == Tier.TIER_0 and env == "prod":
        return GateDecision(False, True,
                            "BLOCK(human): tier-0 service in prod — requires human approval")

    # --- REQUIRE_HUMAN ---
    if env == "prod":
        return GateDecision(False, True,
                            "REQUIRE_HUMAN: production action — propose and wait for approval")
    if budget_remaining < LOW_BUDGET_THRESHOLD:
        return GateDecision(False, True,
                            f"REQUIRE_HUMAN: budget low ({budget_remaining:.0%} left) — escalate")
